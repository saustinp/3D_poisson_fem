            ...
            if elem[0] >= 42:        # Don't need the normal vectors for the outer boundary
                ...
                for tet in mesh_t:
                    ...
                    if np.all(np.any(bl, axis=1)):
                        ...
                        surf_elem[i_elem, -3:] = np.squeeze(nvec*-np.sign(np.dot(nvec, outward_vec)))   # NOTE THE NEGAITVE SIGN HERE! Will show normal pointing out of mesh, correct for outer boundary
                        break
...
if __name__ == '__main__':
    ...
    with open(filename + 'EXPERIMENTAL.npy', 'wb') as f:
        np.save(f, p)
        np.save(f, t)
        np.save(f, pg)

    print('Saved to ' + filename + 'EXPERIMENTAL.npy')